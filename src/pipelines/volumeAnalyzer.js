...
class VolumeAnalyzerPipeline {
  ...
  static async iterateThroughCoins(
    interval,
    length,
    params,
    usdtList,
    ...
    console.log(
      `Starting to Analyze the volume of ${
        usdtList.length
      } coins at ${new Date().toISOString()} with params interval: ${interval} length:${length}`
    );
    for (let index = 0; index < usdtList.length; index++) {
      let volume = new Volume();
      let volumeInfo;
      let response = await Polygon.getCandlesAsync(
        usdtList[index],
        interval,
        length
      );
      ...
      // console.info(`loging volumen info ${JSON.stringify(volumeInfo)}`);
      ...
      if (volumeInfo) {
        if (volumeInfo.isVolumeHigher) {
          ...
          if (
            volumeInfo.percentageIncrement > 250.0 &&
            volumeInfo.percentageIncrement < 500.0
          ) {
            let alertLog = `[INFO ${new Date().toISOString()}] VALUE LARGER THAN 250 for ${
              volumeInfo.coin
            } with an incremental percentage value of ${
              volumeInfo.percentageIncrement
            }${params}`;
          ...
          if (
            volumeInfo.percentageIncrement > 500.0 &&
            volumeInfo.percentageIncrement < 1000.0
          ) {
            let alertLog = `[INFO ${new Date().toISOString()}] VALUE LARGER THAN 500 for ${
              volumeInfo.coin
            } with an incremental percentage value of ${
              volumeInfo.percentageIncrement
            }${params}`;
          ...
          if (
            ...
            let alertLog = `[INFO ${new Date().toISOString()}] VALUE LARGER THAN 1000 ${
              volumeInfo.coin
            } with an incremental percentage value of ${
              volumeInfo.percentageIncrement
            }${params}`;
