...
class Volume {
  ...
  getVolume(response, coin, interval) {
    ...
    if (count > 0) {
      ...
      let volumeIncrementPercentage = this.getVolumeIncrementPercentage(
        volumeList,
        lastVolume
      );
      // create VolumeAnalysis instance
      let isVolumeHigher = lastVolume > volumeIncrementPercentage;
      volumeInfo = new VolumeAnalysis(
        coin,
        isVolumeHigher,
  ...
  getAverageOfVolume(volumeList) {
    let avg;
    if (volumeList > 0)
      avg = volumeList.reduce((a, b) => a + b) / volumeList.length;
    return avg;
  }
